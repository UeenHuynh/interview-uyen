...
\subsubsection*{Phase 3: Voting Ensemble}
A soft-voting ensemble combining 4 models (LR + SVM + RF + XGB) achieves the best single-approach performance:
...
\begin{itemize}
  \item F1 Macro: \textbf{0.473 $\pm$ 0.065}
  \item Per-class F1: Control=0.512, Breast=0.482, CRC=0.456, Gastric=0.338, Liver=0.640, Lung=0.421
\end{itemize}
...
\subsubsection*{Phase 4: Specialist Ensemble (Final Model)}
To address poor performance on Gastric class, binary specialist classifiers are added:

\begin{table}[H]
\centering
\caption{Final model performance metrics: Voting + Specialists ($\alpha=0.8$)}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Mean $\pm$ Std} & \textbf{95\% CI} \\
\midrule
F1 Macro & \textbf{0.475 $\pm$ 0.044} & [0.437, 0.513] \\
Accuracy & 0.471 $\pm$ 0.047 & [0.430, 0.512] \\
AUC (macro, OvR) & \textbf{0.794 $\pm$ 0.026} & \textbf{[0.771, 0.817]} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Model comparison: Voting vs Voting + Specialists}
\begin{tabular}{lccc}
\toprule
\textbf{Approach} & \textbf{F1 Macro} & \textbf{Gastric F1} & \textbf{std} \\
\midrule
Voting Ensemble & 0.473 $\pm$ 0.065 & 0.338 & 0.065 \\
\textbf{Voting + Specialists} & \textbf{0.475 $\pm$ 0.044} & \textbf{0.378} & \textbf{0.044} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Best model:} Voting Ensemble + Specialist classifiers ($\alpha=0.8$)
  \item \textbf{Final F1 Macro:} 0.475 $\pm$ 0.044
